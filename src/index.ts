...
function writeNChar(value: number, n: number = 2) {
    const c = Array(n).join('0') + value;
...
let dt = new Date();
const dts = writeNChar(dt.getFullYear(), 4)
...
const doc = new PDFDocument({ autoFirstPage: true, size: 'Letter' });
...
doc
    ...
    .text(
        ...
        + '.'
        + writeNChar(dt.getFullYear(), 4)
        , 410, 10, { align: 'right', width: 100, height: 50});
